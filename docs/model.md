...
Consumers that require figure controls should treat the artifact set as all-or-nothing: `config.yml`,
`all/counts.csv`, `all/profile.json`, and `by_sample.jsonl` must all exist. Workflows that do not require figure
conditioning may proceed when no figure artifacts exist, but a partially written figure artifact set is invalid.
