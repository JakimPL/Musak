...
The harmonic-plan conditioning layer is not wired into dataset examples or the model yet. `musak_model.conditioning.harmony`
can decode a segment through the existing `ViterbiChordDecoder`, convert decoded chord windows into
`HarmonicPlanWindow` objects, and map each decoder step to low-cardinality IDs for:
...
produce the unknown harmonic-plan ID. Phase 4 will add dataset, batch, and model conditioning for these tensors.
