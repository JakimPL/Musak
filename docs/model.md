...
The harmonic-plan conditioning layer currently defines schema and vocabulary primitives only; it is not wired into
dataset examples or the model yet. `musak_model.conditioning.harmony` converts decoded chord windows into
`HarmonicPlanWindow` objects and maps each window to low-cardinality IDs for:
...
rather than a full tonal analysis. Phase 3 will align these plan windows to decoder-step coordinates, and Phase 4 will
add model conditioning.
