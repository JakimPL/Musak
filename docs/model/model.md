...
FINETUNE_DATA_DIR=data/finetuning-dataset FINETUNE_DIFFICULTY_LABELS=data/finetuning-difficulty.json make finetune
PRETRAIN_DATA_DIR=data/pretraining-dataset FINETUNE_DATA_DIR=data/finetuning-dataset FINETUNE_DIFFICULTY_LABELS=data/finetuning-difficulty.json make train
```
...
```text
artifacts/processed/<dataset-name>/
  parsed.csv
...
Training receives dataset roots. With `--data-dir data/example-dataset`, the training code looks for reusable encoded
artifacts under `artifacts/processed/example-dataset/encoded/<tokenizer-hash>`. Encoded artifacts are used only when
...
Training examples derive additional decoder-step coordinate tensors from the encoded token stream. These coordinates
are model inputs, not persisted encoded artifact fields yet:

- `bar_relative_ticks`: active-hand cursor position inside the current bar, represented as integer ticks over the
  active duration vocabulary denominator.
...
coordinate tensors align with the teacher-forced input sequence `[BOS] + token_ids[:-1]`. Generation uses the same
derivation for `[BOS] + generated_prefix`. The coordinate extractor is permissive and does not replace hard validity
constraints; hard generation constraints remain the authority for exact measure completion and legal continuations.
The coordinate path also tracks internal decoder-step `bar_indices` so harmonic-plan alignment can use the same
post-prefix cursor state as generation. These `bar_indices` are not model inputs yet.

The harmonic-plan conditioning layer can decode a segment through the existing `ViterbiChordDecoder`, convert decoded
chord windows into `HarmonicPlanWindow` objects, and map each decoder step to low-cardinality IDs for:
...
extensions and altered extensions are left out until the decoder can justify them. The default chord-decoding
vocabulary still enables triads only, because seventh chords tie with some simpler triads under pure chord-tone
membership when the seventh is not actually sounding. Harmonic function is derived from
`musak_shared.elements.HARMONIC_FUNCTION_BY_DEGREE`, so the current function label is a coarse diatonic-area hint
rather than a full tonal analysis.

Harmonic-plan alignment uses the decoder coordinate cursor: `bar_start(bar_index) + bar_relative_ticks /
duration_tick_denominator`. This makes the aligned plan describe the score position where the next token would act,
including pickup and short-final-bar durations from `GenerationConstraints`. Padding coordinates and gaps in the plan
...
large-leap recovery, delayed hand responses, synchronized onsets, static long left-hand support under moving right
hand, and final-position closure. Whole-bar metrics are relative to the current bar duration, so pickup and short
final bars are measured separately from absolute `1/1` whole-note durations. These metrics are diagnostics only and
are not part of the sample penalty, sampling constraints, or training objective.
...
`config.yml`, `all/counts.parquet`, `all/profile.json`, and `by_sample.jsonl` must all exist. Workflows may proceed when no
...
against a selected `figure/all/counts.parquet`, but figure profiles are not generation constraints and do not affect
sampling.
