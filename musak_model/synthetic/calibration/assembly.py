...
def build_calibration_generator(config: CalibrationConfig) -> SegmentGenerator:
    ...
    return SegmentGenerator(
        ...
        hand_coupling_sampler=HandCouplingSampler(config=HandCouplingConfig.load()),
        chord_track_sampler=ChordTrackSampler(
            model=uniform_transition_model(chords, self_transition_bias=config.self_transition_bias)
        ),
