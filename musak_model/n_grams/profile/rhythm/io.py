...
def write_rhythm_counts_csv(counts: RhythmCountCounter, path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=RHYTHM_COUNT_CSV_COLUMNS)
        writer.writeheader()
        writer.writerows(_rhythm_count_records(counts))


def read_rhythm_counts_csv(path: Path) -> RhythmCountCounter:
...
def _rhythm_count_records(counts: RhythmCountCounter) -> Iterator[dict[str, str | int]]:
