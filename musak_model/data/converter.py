...
_MIDI_OCTAVE_OFFSET: Final[int] = 1
_PITCH_CLASSES_PER_OCTAVE: Final[int] = 12
...
def pitch_to_degree(
    ...
    *,
    ...
    pitch_class = midi_pitch % _PITCH_CLASSES_PER_OCTAVE
...
def _pitch_class_to_degree(pitch_class: int, *, key_root: int, scale_type: ScaleType) -> tuple[int, int]:
    intervals = SCALE_INTERVALS[scale_type]
    relative = (pitch_class - key_root) % _PITCH_CLASSES_PER_OCTAVE

    if relative in intervals:
        return intervals.index(relative) + 1, 0

    for index, interval in enumerate(intervals):
        if (interval + 1) % _PITCH_CLASSES_PER_OCTAVE == relative:
            return index + 1, 1
...
def _compute_octave_offset(midi_pitch: int, *, hand: Hand) -> int:
    octave = midi_pitch // _PITCH_CLASSES_PER_OCTAVE - _MIDI_OCTAVE_OFFSET
