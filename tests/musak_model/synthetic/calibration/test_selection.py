...
def _result(lambda_curve: float, lambda_harm: float, lambda_accent: float, tv: float | None) -> SweepResult:
    return SweepResult(
...
def test_select_tilts_picks_largest_under_threshold_per_direction() -> None:
    ...
    selection = select_tilts(results, lambda_curve=_GRID, lambda_harm=_GRID, lambda_accent=_GRID, threshold=0.1)

    assert selection.lambda_curve == 1.0
    assert selection.lambda_harm == 1.0
...
def test_select_tilts_reports_threshold_met_when_all_directions_clear() -> None:
    ...
    assert (selection.lambda_curve, selection.lambda_harm, selection.lambda_accent) == (0.5, 0.5, 0.5)
...
def test_select_tilts_skips_missing_distances() -> None:
    ...
    selection = select_tilts(
        results, lambda_curve=(0.0, 0.5), lambda_harm=(0.0, 0.5), lambda_accent=(0.0, 0.5), threshold=0.1
    )

    assert selection.lambda_curve == 0.5
    assert selection.lambda_harm == 0.0
