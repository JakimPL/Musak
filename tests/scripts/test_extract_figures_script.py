...
from musak_model.n_grams.profile.extraction import copy_analysis_config
from scripts.extract_figures import (
    dataset_name_for_analysis,
    default_output_path,
    encoded_run_directories,
    resolve_encoded_directory,
)


def test_resolve_encoded_directory_uses_single_processed_run(tmp_path: Path) -> None:
    data_dir = tmp_path / "data" / "PDMX"
    encoded_directory = tmp_path / "processed" / "PDMX" / "encoded" / "abc"
    _write_encoded_run(encoded_directory)

    resolved = resolve_encoded_directory(
        data_directory=data_dir,
        processed_root=tmp_path / "processed",
        encoded_directory=None,
    )

    assert resolved == encoded_directory


def test_resolve_encoded_directory_requires_override_for_multiple_runs(tmp_path: Path) -> None:
    data_dir = tmp_path / "data" / "PDMX"
...
def test_resolve_encoded_directory_prefers_explicit_encoded_directory(tmp_path: Path) -> None:
    encoded_directory = tmp_path / "custom"
...
def test_encoded_run_directories_requires_jsonl_and_tokenizer_snapshot(tmp_path: Path) -> None:
    ...
    assert encoded_run_directories(tmp_path / "encoded") == [valid]
